...
(async()=>{
  ...
  try {
    ...
    const pack=process.argv[2] || 'artifacts/tap-inspector-0.3.7-built/pack';
    ...
    await page.evaluate(()=>{
      ...
      window.TapBridge={isReady:()=>false,expose:(name,handler)=>{window.fixtureOperations.set(name,handler);return()=>window.fixtureOperations.delete(name);},status:()=>({state:window.fixtureBridgeState,plan_state:window.fixturePlanState,pending:window.fixturePending,activity:window.fixtureActivity,actions:[],packs:[{id:'fixture.reader',version:'2.0.0',features:[{id:'archive',label:'Session archive',value:'Versioned JSON'}]},{id:'fixture.ui',version:'1.2.3',features:[]},{id:'tap.inspector',version:'0.3.7',features:[{id:'page-inspection',label:'Page inspection',value:'Local WebSocket operations'}]}]})};
